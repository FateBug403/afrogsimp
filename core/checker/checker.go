...
import (
	...
	"github.com/FateBug403/afrogsimp/core/protocols/gox"
	"github.com/FateBug403/afrogsimp/core/protocols/http/retryhttpclient"
	"github.com/FateBug403/afrogsimp/core/protocols/netxclient"
	"github.com/FateBug403/afrogsimp/core/protocols/raw"
	...
	"github.com/FateBug403/afrogsimp/core/proto"
	"github.com/FateBug403/afrogsimp/core/utils"
...
// 这段比较难以理解，记住三点就是，第一点是设置为ActiveTarget相当于设置检查通过，后续不在检测
// 这个目标，第二点是如果目标检测没有通过，则会调用UpdateNum累计目标错误数量，当错误数量到达
// 我们设置的MaxHostError时，会直接返回空，相当于加入了黑名单，第三点就是全局变量Targets的
// 作用是统计每个目标检测错误的状态内存数据库
func (c *Checker) checkURL(target string) (string, error) {

	// if target check num more than MaxCheckNum
	tcount := Targets.Num(target)
	...
	if !utils.IsURL(target) {
		if newtarget, err := retryhttpclient.CheckProtocol(target); err == nil {
			if k := Targets.Key(target); k >= 0 {
				Targets.Update(k, newtarget)
				Targets.SetNum(newtarget, ActiveTarget)
			}
			return newtarget, nil
		}

		Targets.UpdateNum(target, 1)
		return target, fmt.Errorf("%s check protocol falied", target)
	}

	// if target is url more than zero, then check protocol against
	if Targets.Num(target) >= 0 {
		if newtarget, err := retryhttpclient.CheckProtocol(target); err == nil {
			Targets.SetNum(newtarget, ActiveTarget)
			return newtarget, nil
		}
...
func (c *Checker) UpdateVariableMap(args yaml.MapSlice) {
	for _, item := range args {
		...
		case map[string]string:
			...
			c.CustomLib.UpdateCompileOption(key, mycel.StrStrMapType)
		default:
